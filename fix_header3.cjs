...
const targetStr = `<div className="flex items-center gap-2 order-3 sm:order-2 overflow-x-auto no-scrollbar max-w-full">
                {/* 3 Main View Modes Tabs (Clean, fully responsive) */}
                {spatialMode === '3d' && (
                  <div className="flex border border-[#1B1813] bg-white p-0.5 shrink-0 shadow-xs">
                    {[
...
const newHeader = `{/* 4 Main View Modes Tabs (Clean, fully responsive) */}
              <div className="flex border border-[#1B1813] bg-white p-0.5 overflow-x-auto no-scrollbar max-w-full order-3 sm:order-2 shrink-0">
                {[
                  { id: 'assembled', label: '01 · MONOLÍTICO' },
                  { id: 'exploded', label: '02 · DESPIECE VERTICAL' },
                  { id: 'unit', label: '03 · AISLAR UNIDAD' },
...
content = content.replace(targetStr, newHeader);
fs.writeFileSync(file, content);
console.log('Fixed header 3');
